...
- (void)setLeftValue:(NSInteger)leftValue {
    ...
    if (leftValue <= self.minValue) {
        ...
        _leftOrigin = _leftHandle.frame.origin.x;
    }
...
- (void)setRightValue:(NSInteger)rightValue {
    ...
    if (rightValue >= self.maxValue) {
        ...
        _rightOrigin = _rightHandle.frame.origin.x;
    }
...
- (UIImage *)handleImage {
    if(!_handleImage) {
        UIImage *image = [UIImage imageNamed:@"SliderHandle"];
        _handleImage = image;
    }
    return _handleImage;
}
...
- (UIImage *)trackBackgroundImage {
    if(!_trackBackgroundImage) {
        UIImage *image = [[UIImage imageNamed:@"SliderBase"] resizableImageWithCapInsets:UIEdgeInsetsMake(4, 5, 4, 5)];
...
- (UIImage *)trackFillImage {
    if(!_trackFillImage) {
        UIImage *image = [[UIImage imageNamed:@"SliderTrack"] resizableImageWithCapInsets:UIEdgeInsetsMake(4, 5, 4, 5)];
