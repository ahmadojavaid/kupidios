...
- (void)archiveMessage:(XMPPMessage *)message outgoing:(BOOL)isOutgoing xmppStream:(XMPPStream *)xmppStream
{
    ...
    NSLog(@"Compossing and Ou going %d   from %@",isOutgoing,[message from].bareJID);
	...
	if ([messageBody length] == 0)
	{
        ...
        if (!isOutgoing) {
            ...
            if (isComposing)
            {
                if ([[message from].bareJID isEqualToJID:[XMPPJID jidWithString:self.otherjid]]) {
                    if (self.delegate) {
                        [self.delegate ComposeStatus:@"Typing..."];
                    }
                }
            }
            ...
            if (isPaused) {
                if ([[message from].bareJID isEqualToJID:[XMPPJID jidWithString:self.otherjid]]) {
                    if (self.delegate) {
                        [self.delegate ComposeStatus:@"Paused..."];
                    }
                }
            }
...
//
//
//            isComposing = [message hasPausedChatState];
//            if (!isComposing)
//            {
